...
The Karaf Shell is exposed on TCP port 8201.
...
By default, Minion will run using the default `minion` user (uid: 999, gid: 997). Fortunately, thanks to OpenJDK 11 and the `setcap` utility, it is possible to execute ICMP requests without requiring changes on kernel settings.
